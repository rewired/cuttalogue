...
- Phase 7 now provides typed Shot and Camera Direction writes plus scene assignment, finite 3D anchor upserts, and camera-target binding over atomic persistence, mandatory expected revisions, structured conflicts, HTTP parity, and MCP mutation metadata. Shots can be spatially directed without raw JSON access; asset, constraint, prompt-save, and generation controls remain planned.
